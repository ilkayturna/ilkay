...
const ChatWidget: React.FC<ChatWidgetProps> = ({ language }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const botMsg: ChatMessage = {
        ...
        role: 'model',
        text: responseText || t.chat.noResponse,
        timestamp: new Date()
      };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[350px] sm:w-[400px] h-[500px] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700 animate-fade-in-up">
          {/* Header */}
          <div className="bg-slate-900 dark:bg-slate-950 text-white p-4 flex justify-between items-center shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-300">{t.chat.poweredBy}</p>
              </div>
            </div>
            <button 
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 bg-slate-50 dark:bg-slate-900 space-y-4 scrollbar-thin">
            {messages.map((msg) => (
              <div
                key={msg.id}
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 rounded-2xl rounded-tl-none px-4 py-3 shadow-sm">
          ...
          <form onSubmit={handleSend} className="p-3 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500 dark:placeholder-slate-400"
            />
            <button
              type="submit"
